...
pub async fn get_orders(req: HttpRequest) -> HttpResponse {
    ...
    if let Some(_cookie) = req.cookie("token") {
        ...
        let user_orders = orders.load(conn).expect("Error finding orders");
...
pub async fn get_order(path: Path<String>, req: HttpRequest) -> HttpResponse {
    ...
    if let Some(_cookie) = req.cookie("token") {
        ...
        let order = orders.filter(id.eq(get_order_id)).load(conn).expect("Error finding order");
...
pub async fn update_order(
    ...
    if let Some(_cookie) = req.cookie("token") {
        ...
        let update_order = diesel
            ::update(orders.filter(id.eq(get_order_id)))
...
pub async fn delete_order(path: Path<String>, req: HttpRequest) -> HttpResponse {
    ...
    if let Some(_cookie) = req.cookie("token") {
        ...
        let delete_order = diesel::delete(orders.filter(id.eq(get_order_id))).execute(conn);
